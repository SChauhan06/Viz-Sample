...
#' @export
style_base <- function(textsize=9) {
  list(
...
#' @export
style_atlas <- function(textsize=9, family="Avenir Book", family.bold ="Avenir Heavy", is.cmyk = FALSE) {
    ...
    theme = function() {
      theme_minimal() +
        theme(text                 = element_text(family = family, size=textsize, color=colors$text),
              ...
              strip.text           = element_text(hjust = 0, size=5, lineheight = 1, color=colors$text), # strip text is NOT kind of like subtitle text
...
#' @export
style_atlas_open <- function(textsize=9, ...) {
